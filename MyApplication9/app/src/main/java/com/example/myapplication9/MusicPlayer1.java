...
public class MusicPlayer1 extends Service {
    ...
    class MusicControl extends Binder{
        public void play() {//开始播放
            try{
                ...
                player = MediaPlayer.create(getApplicationContext(),R.raw.daoxiang); //加载多媒体文件
                player.start(); //开始播放音乐
                addTimer();//添加计时器
        ...
        if (timer == null){
            ...
            TimerTask task = new TimerTask() {//实例化一个计时任务对象
                ...
                public void run() { //run就是多线程的一个东西,用于service和主线程（即MainActivity）之间的通信
                    ...
                    Message message = song1.handler.obtainMessage();//在主线程获取一个消息空间
                    ...
                    song1.handler.sendMessage(message);//将消息添加到主线程中
                }
            };
